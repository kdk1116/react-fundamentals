...
The important part of the exercise is to know how the side effects apply to the state
in React. One of the limitations of React is that when using an array if you don't initialize
a key prop to each element that is unique to the elemnet. You will arrive to a side effect where
React will keep mounting and unmounting states but not the one that you would want. It will be based
off the index which could lead to a very bad problem when rendering lists and arrays
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=07%3A%20Rendering%20Arrays&em=kevin.d.kim1116%40gmail.com).
